  ...
  def order_grasps_noisy(self):
    ...
    for i, grasp in enumerate(self.grasps_ordered_noisy):
      ...
      for j in range(M):
        ...
        noisy_evaluations[j] = self.eval_grasp(grasp)
     
      
      #take the mean of the M trials
      grasp[self.graspindices.get('performance')] = np.mean(noisy_evaluations)
