...
class RoboHandler:
  ...
  def order_grasps(self):
    ...
    for i, grasp in enumerate(self.grasps_ordered):
      print round((100.0*i)/len(self.grasps_ordered), 0), '%'
  ...
  def order_grasps_noisy(self):
    ...
    for i, grasp in enumerate(self.grasps_ordered_noisy):
      print round((100.0*i)/len(self.grasps_ordered_noisy), 0), '%'
      scores = [self.eval_grasp(self.sample_random_grasp(grasp)) for j in range(3)]
      grasp[self.graspindices.get('performance')] = min(scores)
  ...
  def eval_grasp(self, grasp):
    with self.robot:
      ...
      try:
        ...
        # print 'objet pose'
        # print self.gmodel.target.GetTransform()
        # for each contact
        ...
        for i, c in enumerate(contacts):
          ...
          r = np.array([pos])
          n = np.array([dir])
          wrench = np.transpose(np.hstack((n, np.cross(r, n))))
          # print 'wrench'
          # print wrench
          G[:,i:i+1] = wrench
        ...
        score = min(s)*LA.det(np.dot(G, np.transpose(G))) #min(s)**2/max(s)
        # print 'score'
        # print score
...
#            finestep              = None )
  ...
  def sample_random_grasp(self, grasp_in):
    ...
    grasp[self.graspindices['igrasppos']] = pos_orig + np.random.randn(*pos_orig.shape)*RAND_DIST_SIGMA
    ...
    grasp[self.graspindices['igraspdir']] = dir_orig + np.random.randn(*dir_orig.shape)*RAND_ANGLE_SIGMA
    grasp[self.graspindices['igrasproll']] = roll_orig + np.random.randn(*roll_orig.shape)*RAND_ANGLE_SIGMA
